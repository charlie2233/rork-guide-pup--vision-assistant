...
7. Inspect the `.xcarchive`, exported IPA when present, signing identity class, entitlements, merged privacy manifests, production environment, and absence of a Sentry SDK/DSN/Crash Data manifest. Generate `expo/release/candidate-build.latest.json` with `release-candidate-evidence.mjs`, passing the expected app version, build number, bundle identifier, team identifier, and source revision.
